...
const fileFilter = (req, file, cb) => {
  const allowTypes = ['image/jpeg', 'image/png', 'image/gif', 'image/webp', 'image/bmp'];
  if (allowTypes.includes(file.mimetype)) {
    cb(null, true);
  } else {
    // 自定义错误（带 statusCode，交给全局错误处理器返回 400）
    const err = new Error('只能上传图片文件（jpg/png/gif/webp/bmp）');
    err.statusCode = 400;
    cb(err);
  }
};
